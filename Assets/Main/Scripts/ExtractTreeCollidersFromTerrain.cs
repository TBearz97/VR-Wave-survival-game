...
public class ExtractTreeCollidersFromTerrain : MonoBehaviour
{
    ...
    private void CreateNavMeshObstacle(TreePrototype tree, int prototypeIndex, TreeInstance[] instances, int instanceIndex)
    {
        ...
        var obj = CreateGameObjectForNavMeshObstacle(tree, instances, instanceIndex, primitiveScale, navMeshObstacle);
        _createdObjects.Add(obj);
    }

    private GameObject CreateGameObjectForNavMeshObstacle(TreePrototype tree, TreeInstance[] instances, int instanceIndex, Vector3 primitiveScale, NavMeshObstacle meshObstacle)
    {
        GameObject obj = null;
        if (meshObstacle.shape == NavMeshObstacleShape.Capsule)
        {
        ...
        obj.GetComponent<NavMeshObstacle>().size = primitiveScale;
